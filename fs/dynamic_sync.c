...
/*
 * Use DRM notifier to automatically enable file synchronization after LCD
 ...
 * states quite often. If DRM notifier is unused in the kernel, this will
 * be dropped.
 */
#define DEF_DRM_NOTIFIER_IS_USED	(1)
 ...
 * usage of DRM notifier and a timer fire after dyn_sync.delay milliseconds.
				 ...
				 unsigned long event, void *data)
{
	...
	if (likely(!dyn_sync.enabled || !dyn_sync.drm_notify_used))
		return NOTIFY_OK;
					  ...
					  const char *buf, size_t count)
{
	 ...
	 * If DRM notifier is not used, just change the synchronization
				     ...
				     const char *buf, size_t count)
{
	 ...
	 * available instantly after DRM notify enablement without the
	 * requirement to turn the screen on/off. In case user wants to throw
	 * DRM notify support away, just disable fsync as he might expect this
	 * to happen.
	 */
	 ...
	 * if DRM notify is going to be dropped.
	 */
