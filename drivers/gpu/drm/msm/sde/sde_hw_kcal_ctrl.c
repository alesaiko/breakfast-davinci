...
static void sde_hw_kcal_pcc_adjust(u32 *data, int plane)
{
...
static struct drm_msm_pa_hsic sde_hw_kcal_get_hsic(void)
{
...
static struct sde_hw_kcal_pcc kcal_pcc = {
	.red	= SDE_HW_KCAL_INIT_PCC,
	.green	= SDE_HW_KCAL_INIT_PCC,
	.blue	= SDE_HW_KCAL_INIT_PCC,
};

static struct sde_hw_kcal_hsic kcal_hsic = {
...
static struct sde_hw_kcal_ops kcal_ops = {
	.adjust_pcc	= sde_hw_kcal_pcc_adjust,
	.get_hsic	= sde_hw_kcal_get_hsic,
};

static struct sde_hw_kcal kcal_lut_data = {
	.enabled	= SDE_HW_KCAL_ENABLED,
	.min_cap	= SDE_HW_KCAL_MIN_VALUE,
	.pcc		= &kcal_pcc,
	.hsic		= &kcal_hsic,
	.ops		= &kcal_ops,
};
			  ...
			  const char *buf, size_t count)
{
	...
	pcc->red   = max(kcal_r, kcal_lut_data.min_cap);
	pcc->green = max(kcal_g, kcal_lut_data.min_cap);
	pcc->blue  = max(kcal_b, kcal_lut_data.min_cap);
...
define_one_kcal_node(kcal_min, min_cap, 1, 256);
